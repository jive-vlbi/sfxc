...
void test_output_node(int rank, int numtasks,
  ...
  if (rank == RANK_MANAGER_NODE) {
    ...
    }
    {
      ...
      MPI_Send(msg, size, MPI_CHAR, 
               rank_output_node, MPI_TAG_ADD_DATA_WRITER_FILE2, MPI_COMM_WORLD);
      wait_for_setting_up_channel(rank_output_node);
    }

    int slicenr = 0;
    { // Sending slice to the output_node
      int nBytes = 1000;
...
int main(int argc, char *argv[]) {
  assert(rank_output_node+RANK_MANAGER_NODE+RANK_LOG_NODE == 3);
