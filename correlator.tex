...
signal has to be processed further. \acomment{I like the details of
  the algoritm.  Could this be separated to its "distribution" on
  multiple node}{}First, the signal is delayed by a the fractional
...
approximation of the delay function. \acomment{Maybe move to
  "network/distribution" part}{These manipulations are not done on the
  input node because they require floating point samples, hence the
  data stream expands from 2 bits per sample to 32 bits or even 64
  bits per sample, which would require much more network bandwidth}.
...
\subsection{Network connections}\marginpar{Remove this section?}
...
very important as we are dealing with real time data. \acomment{NGHK:
  Maybe later We should come with a formula linking buffer-size,
  packet-loss throughput and bandwidth}{Some packet loss is even
  acceptable if a data stream at a constant rate can then be
