...
\acomment{Ok I understand now why you need eVLBI before...}{}
...
\acomment{added.}{software} correlator using a computing Grid. The advantages 
of a software correlator over a new dedicated hardware correlator lie in its
flexibility and \acomment{why a software correlator would be 
more accurate ?}{accuracy}. \acomment{added.}{Thus t}he software correlator can be tuned for
special experiments. The main advantage of a dedicated hardware
correlator is the \acomment{greater efficiency ?}{greater performance}. 
...
\acomment{Copyied here to groupe all the stuff about flexibility 
together.}{The advance of general purpose 
...
have to be correlated. \acomment{ I disgagree with the logical 
organization of causes/consequence here.}{The huge need for networking 
and computing power makes a computing Grid an ideal platform for 
this application.}

\acomment{This should be in Design section}{
...
\subsection{Design}
\acomment{Is this really so important that we need to start with this ? In addition you don't explain why we need FFTs, why spline fitting etc..}{}The software correlator is written in \verb~C++~ and uses several
...
\acomment{The following is not "design".}{}At start up, the manager node is created, which starts the other nodes
...
signal has to be processed further. \acomment{I like the details of the algoritm. 
Could this be separated to its "distribution" on multiple node}{}First, the signal is delayed by a
...
order approximation of the delay function. \acomment{Maybe move to "network/distribution" part}{These manipulations are not
...
\acomment{I disagree, correlation is a computation problem. It is when 
we distribute it that is become a networking problem.}{}
...
delivery of the messages has to be guaranteed. \acomment{If it has no importance maybe we could reduce the size of this part?}{For the analysis of the
...
very important as we are dealing with real time data. \acomment{We should come with a formula linking buffer-size, packet-loss throughput and bandwidth}{Some packet loss
...
protocols like TCP \acomment{sound like a technical buzzword/magicword for astronomers :)}{(using jumbo frames)}, 
